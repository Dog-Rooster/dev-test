<x-guest-layout>
    <div class="container mx-auto py-8">
        @if (!request('booking_time'))
            <h1 class="text-2xl font-bold mb-0">Select a Time Slot for {{ $event->name }}</h1>
            <h3 class="text-1xl mb-6">8.am - 5.pm Monday - Friday on EDT</h3>

            <div class="mb-4">
                <form action="{{ route('bookings.create', $event->id) }}" method="GET" class="flex gap-4 items-end">
                    <div>
                    ...
                    <div >
                        ...
                        <select id="booking_timezone" name="booking_timezone" class="">
                            @foreach ($timeZones as $timezone)
                                <option value="{{$timezone['value']}}" {{$timezone['value'] == $selectedTimeZone ? 'selected' : ''}} >{{$timezone['label']}}</option>
                            @endforeach
                        </select>
                    </div>
                    <button type="submit" class="h-12 ml-4 px-4 py-2 bg-blue-600 text-white rounded">Change Date & TimeZone</button>

                </form>
            </div>
            @if (count($timeSlots) == 0)
                <p class="text-gray-600">No time slots are available.</p>
            @else
